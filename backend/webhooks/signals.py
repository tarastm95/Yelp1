...
def notify_new_lead(sender, instance: LeadDetail, created: bool, **kwargs):
    ...
    if business:
        ...
        if len(settings) > 1:
            logger.info(f"[SMS-NOTIFICATION] 📱 Multiple NotificationSettings found ({len(settings)}), sending to ALL numbers")
            logger.info(f"[SMS-NOTIFICATION] This ensures all configured recipients get notifications")
            ...
            logger.info(f"[SMS-NOTIFICATION] All {len(settings)} settings will be used for SMS sending")
    else:
