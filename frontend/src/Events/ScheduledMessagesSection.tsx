...
// Material-UI компоненти (без Grid та Snackbar)
import {
  Typography,
  TextField,
  Button,
...
// Grid і Snackbar — окремо через default-імпорт
...
const ScheduledMessagesSection: FC<ScheduledSectionProps> = ({
  ...
  const handleAdd = async () => {
    ...
    setError(null);
    try {
      const token = localStorage.getItem('yelp_access_token');
      await axios.post(
      ...
      <Paper sx={{ p: 2, mb: 3 }}>
        ...
        <Paper variant="outlined" sx={{ p: 2, mb: 3 }}>
          ...
          <Grid container spacing={1} sx={{ mb: 2 }}>
            <Grid item>  {/* тепер працює без помилок */}
              <Chip
                icon={<PersonPinIcon />}
                label="Insert Name"
